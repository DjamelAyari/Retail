...
{
	printf("Entrée dans la fonction queries_seed_vegetable_prepared_statement !\n");
	...
	char query_seed_param[4095] = "SELECT name_vegetable, variety_vegetable, latin_name_vegetable, origine_seed, price_seed, quantity_stock, month_gathering, month_cultivation, floor_type, quantity_sun_need, quantity_water_need "
                   "FROM vegetable "
	...
	while (i < array_size)
	{
		...
		{
			strcat(query_seed_param, " AND name_vegetable = ?");
		...
		{
			strcat(query_seed_param, " AND variety_vegetable = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_seed_param);
			j++;
		}
		if (strcmp(array[i].key, "latin_name_vegetable") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_seed_param, " AND latin_name_vegetable = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_seed_param);
			j++;
		}
