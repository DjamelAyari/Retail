...
{
	printf("Entry in the queries_edible_preapred_statement() function !\n");
	...
	char query_edible_param[4095] = "SELECT name_fruit, variety_fruit, latin_name_fruit, origine_edible, price_edible, quantity_stock, month_gathering "
                   "FROM fruit "
                   "JOIN edible ON fruit.id_fruit = edible.id_fruit "
	...
	while (i < array_size)
	{
		...
		{
			strcat(query_edible_param, " AND name_fruit = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "variety_fruit") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND variety_fruit = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "latin_name_fruit") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND latin_name_fruit = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "origine_edible") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND origine_edible = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "price_edible") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND price_edible = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "quantity_stock") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND quantity_stock = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "month_gathering") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND month_gathering = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
	...
	if (mysql_stmt_prepare(stmt, query_edible_param, strlen(query_edible_param)) != 0)
	{
