...
{
	printf("Entrée dans la fonction queries_edible_vegetable_prepared_statement !\n");
	...
	char query_edible_param[4095] = "SELECT name_vegetable, variety_vegetable, latin_name_vegetable, origine_edible, price_edible, quantity_stock, month_gathering "
                   "FROM vegetable "
	...
	while (i < array_size)
	{
		...
		{
			strcat(query_edible_param, " AND name_vegetable = ?");
		...
		{
			strcat(query_edible_param, " AND variety_vegetable = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
		if (strcmp(array[i].key, "latin_name_vegetable") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_edible_param, " AND latin_name_vegetable = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_edible_param);
			j++;
		}
