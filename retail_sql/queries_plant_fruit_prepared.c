...
{
	printf("Entrée dans la fonction queries_plant_fruit_prepared_statement !\n");
	...
	char query_plant_param[4095] = "SELECT name_fruit, variety_fruit, latin_name_fruit, origine_plant, price_plant, quantity_stock, month_gathering, month_cultivation, floor_type, quantity_sun_need, quantity_water_need "
                   "FROM fruit "
	...
	while (i < array_size)
	{
		...
		{
			strcat(query_plant_param, " AND name_fruit = ?");
		...
		{
			strcat(query_plant_param, " AND variety_fruit = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_plant_param);
			j++;
		}
		if (strcmp(array[i].key, "latin_name_fruit") == 0 && strcmp(array[i].value, "NULL") != 0)
		{
			strcat(query_plant_param, " AND latin_name_fruit = ?");
			printf("Array[i] = %s And Query = %s\n", array[i].key, query_plant_param);
			j++;
		}
