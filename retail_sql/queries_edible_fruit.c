...
{
	printf("QUERIES_2\n");
	printf("Entrée dans la fonction queries_edible_fruit !\n");
	...
	char query_edible[4095] = "SELECT name_fruit, variety_fruit, latin_name_fruit, origine_edible, price_edible, quantity_stock, month_gathering "
                   "FROM fruit "
	...
	while ((row = mysql_fetch_row(result)))
	{
		...
		char *name_fruit = row[0] ? row[0] : "NULL";           // First column: name_fruit
		char *variety_fruit = row[1] ? row[1] : "NULL";        // Second column: variety_fruit
		char *latin_name_fruit = row[2] ? row[2] : "NULL";     // Third column: latin_name_fruit
